...
def run_testcase_filter(
    ...
    for data in dataset:
        ...
        if not offline:
            f.write(json.dumps({"task_id": tid,"completion":output_short,"time_record":time_record,"fix_record":fix_record,"step_one_total_time":step_one_total_time,"step_one_tokens_len":(input_tokens_len,output_tokens_len),"task_time":task_time,"chosen_testcase_dict":chosen_testcase_dict,"pass_testcase_list_dict":pass_testcase_list_dict,"gened_testcase":task_gened_testcase,"internal_tests":chosen_testcase,"chosen_testcase_id_dict":chosen_testcase_id_dict})+"\n")
            f.flush()
        else:

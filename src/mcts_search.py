...
class Node:
    ...
    def best_child(self):
        if not self.children:  # Check if children list is empty
            return None
        return max(self.children, key=lambda child: (child.uct(),child.prob))
...
def run_mcts_search(
    ...
    for data in dataset:
        ...
        s=solutions[0]
        root = Node(s,prompt="",depth=cir,feedbackprompt="")
        root.idx = len(nodes)
        nodes.append(root)
        gened_nodes.append(root)
        ...
        while True:
            ...
            while tmp.children:
                tmp = tmp.best_child()
            chosen_nodes.append(tmp)
            #important
            left_nodes =  [node for node in total_nodes if node not in chosen_ndoes]#sorted_nodes# sorted_nodes[sample_num:]
            #important

...
def run_testcase_filter(
    ...
    for data in dataset:
        ...
        if offline:
            ...
            task_gened_testcase = gen.gen_tests(model,data,num=100,verbose=verbose)[:100]
            print_v(f"gen {len(task_gened_testcase)} tests for task {tid}.")
        
        if data.get('prompt_tests', []) == []:
            print_v(f"task {tid} has no prompt tests.")
            internal_tests = task_gened_testcase[:6]
            print_with_tag(content="\n".join(internal_tests),tag="internal_tests",verbose=verbose)
            assertions = internal_tests
            base_assertions = internal_tests
            base_assertion_string = "\n".join(assertions)
            assertion_string = "\n".join(assertions)
            has_visibale_tests = False
        else :
            print_v(f"task {tid} has prompt tests.")
            assertions = data["prompt_tests"]
            base_assertions = data["prompt_tests"]
            base_assertion_string = "\n".join(assertions)
            assertion_string = "\n".join(assertions)
        ...
        prompt_test_len = len(base_assertions)
        if has_visibale_tests:
            chosen_num = max(0,10-prompt)
        else:
            chosen_num = 6
        chosen_testcase = base_assertions
        ...
        while True:
            ...
            if has_visibale_tests:
                get_pass_rate(data,gened_nodes,base_assertions,tid)
            else:
                get_pass_rate(data,gened_nodes,chosen_testcase,tid)
            ...
            # try:
            sorted_group,chosen_testcase_id  = get_CODET_point_v3(total_nodes,task_gened_testcase,data,sort_len=True,chosen_num=chosen_num) #这里是使用去重后的还是不去重的,sort_len=True
            if chosen_testcase_id!=[]:
                if has_visibale_tests:
                    chosen_testcase = base_assertions + [task_gened_testcase[x] for x in chosen_testcase_id]
                else:
                    chosen_testcase = [task_gened_testcase[x] for x in chosen_testcase_id]
            else:
